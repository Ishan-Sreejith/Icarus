...
.text
    adrp x20, .L1@PAGE
    add x20, x20, .L1@PAGEOFF
    mov x0, x19
    mov x1, x20
    mov x2, x21
    bl _set_struct_member
    mov x0, #20
    str x0, [sp, #24]
    ldr x19, [sp, #8]
    ldr x21, [sp, #24]
...
.text
    adrp x20, .L4@PAGE
    add x20, x20, .L4@PAGEOFF
    mov x0, x19
    mov x1, x20
    bl _get_struct_member
    str x0, [sp, #16]
    ldr x0, [sp, #16]
    bl _print_val
    adrp x1, .newline@PAGE
